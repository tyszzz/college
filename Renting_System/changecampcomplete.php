<html>
	...
	<body>
		...
		$r_pwd=$_GET["r_pwd"];
		$rnu=$_GET["number"];
		$sql_query="select * from rent_rcd where rentStartTime>='".str_replace('-','',$rentStartTime)."'170000 and rentStartTime<='".str_replace('-','',$rentOverTime)."'090000 and r_number!='".$rnu."'";
		...
		$sql_query="select * from rent_rcd where rentStartTime<='".str_replace('-','',$rentStartTime)."'170000 and rentOverTime>='".str_replace('-','',$rentStartTime)."'170000 and r_number!='".$rnu."'";
		$result=mysql_query($sql_query);
		while($row = mysql_fetch_array($result))
		{
			if($temp==1)
		...
		if($temp==0)
		{
			...
			$sql_query="insert into rent_rcd values('".$rnu."','".$numberOfRenters."','".str_replace('-','',$rentStartTime)."170000','".str_replace('-','',$rentOverTime)."090000',CURRENT_TIMESTAMP,'0','0','0','0','".$cnt."','".$r_account."')";
			mysql_query($sql_query);
			if(isset($_GET['camp']))//get會更改網址的值，post的不會
			{
				foreach($_GET['camp']as$key=>$value)
